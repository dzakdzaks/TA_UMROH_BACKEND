...
$sql = mysql_query("SELECT tiket.*,paket_umroh.*,users.*, paket_umroh.id AS id_paket, users.id AS id_user FROM tiket INNER JOIN paket_umroh ON tiket.id_paket=paket_umroh.id INNER JOIN users ON tiket.id_user=users.id WHERE users.id = '$id' ORDER BY id_tiket DESC")or die(mysql_error());
